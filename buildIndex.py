...
class BuildIndex:
    ...
    def makeDirectories(self, file_no):
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/clusters")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/dataset")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/indexed")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/indexed/test")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/KMeansFAISS")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/labels")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/normalizedDataset")
        

    def copyFiles(self, file_no, idx, size):
        for i in range(size):
            # check if the file is found
            if not os.path.exists(f"D:/Boody/GP/witbaseClean/train-{str(i+idx).zfill(5)}-of-00330.parquet"):
                print(f"File train-{str(i+idx).zfill(5)}-of-00330.parquet not found")
                continue
            shutil.copy(f"D:/Boody/GP/witbaseClean/train-{str(i+idx).zfill(5)}-of-00330.parquet", f"Z:/DataImgs/Data_{file_no}/dataset")
            print(f"Copying train-{str(i+idx).zfill(5)}-of-00330 done")
        print("********* Copying done *********")


    def readDataset(self, file_no):   
        path = f"Z:/DataImgs/Data_{file_no}/dataset/"

        parq = pd.read_parquet(path, engine="fastparquet", columns=["id", "embedding"])
        
        # print(embeds["embeddings"][0])
        # normalize the embeddings
        parq['embedding'] = parq['embedding'].apply(lambda x: np.array(x) / np.linalg.norm(x) if x is not None else None)
        # print(embeds["embeddings"][0])
        self.embeds = parq['embedding'].values
    ...
    def writeNormalizedDataset(self, file_no):
        ...
        for i in range(0, len(self.embeds), 100000):
            with open(f"Z:/DataImgs/Data_{file_no}/normalizedDataset/{c}-embeds-batch.txt", "w") as f:
    ...
    def FAISSClustering(self, file_no):
        ...
        faiss.write_index(kmeans.index, f"Z:/DataImgs/Data_{file_no}/KMeansFAISS/index.bin")
        print("********* FAISS Clustering done *********")


    def pointsAssignmentToClusters(self, file_no):
        # read the faiss index
        index = faiss.read_index(f"Z:/DataImgs/Data_{file_no}/KMeansFAISS/index.bin")
        ...
        with open(f"Z:/DataImgs/Data_{file_no}/labels/faiss_labels.txt", "a") as f:
    ...
    def getNearestClusters(self, file_no):
        ...
        with open("Z:/DataImgs/query.txt", "r") as f:
        ...
        # write the labels to a file where I_flat is the centroid index for each embedding
        with open(f"Z:/DataImgs/Data_{file_no}/labels/query_faiss_labels.txt", "w") as f:
            f.write(str(I_flat).replace("[", "").replace("]", "").replace(",", "").replace("  ", " ").strip())
    ...
    def deleteFiles(self, file_no):
        shutil.rmtree(f"Z:/DataImgs/Data_{file_no}/clusters")
        shutil.rmtree(f"Z:/DataImgs/Data_{file_no}/dataset")
        shutil.rmtree(f"Z:/DataImgs/Data_{file_no}/normalizedDataset")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/clusters")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/dataset")
        self.makeDirectory(f"Z:/DataImgs/Data_{file_no}/normalizedDataset")
...
# file_no = 39
for file_no in range(12,14):
    indexer = BuildIndex()

    indexer.makeDirectories(file_no)
    indexer.copyFiles(file_no, file_no * 25, 25)
    ...
    topK = 10
    isImages = 1
    headerPath = "Z:/DataImgs/"
    command = f"{headerFile}cmake-build-debug/RAGn_Roll_Indexer.exe {R} {L} {alpha} {topK} {buildIndex} {headerPath} {file_no} {isImages} -lboost_serialization -lboost_system"
    print(command)
    os.system(command)
